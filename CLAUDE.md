...
- **`validators.ts`** — seven pure validators: `syllableValidator`, `lockedWordsValidator`, `endCollisionValidator`, `fillerEndingValidator`, `heldVowelValidator`, `lengthAlignmentValidator`, `avoidWordsValidator`. `validateLines` aggregates them per line. `heldVowelValidator` triggers on final-note duration ≥ 1.75× median; `lengthAlignmentValidator` triggers on `length === 'L'` (duration > phrase mean) — both check that the final word ends in an open/diphthong vowel.
